...
PATH_TO_CKPT = 'H:/DL projects/shredder-mask mix/hand_detection_tfod-master/frozen_graphs/human_frozen_inference_graph.pb'
# List of the strings that is used to add correct label for each box.
PATH_TO_LABELS = 'H:/DL projects/shredder-mask mix/hand_detection_tfod-master/frozen_graphs/mscoco_complete_label_map.pbtxt'
...
def draw_box_on_image_new(num_hands_detect, score_thresh, scores, boxes, classes, im_width, im_height, image_np):
    ...
    for i in range(boxes.shape[1]):
        ...
        if  (scores[i] > score_thresh):
            ...
            cX = int((left + top) / 2.0)
            cY = int((right + bottom) / 2.0)

            centroid_dict[objectId] = (cX, cY, left, right, top, bottom)
            ...
            if dist:
                hand_cnt = hand_cnt + 1
            cv2.rectangle(image_np, p1, p2, color, 3, 2)
